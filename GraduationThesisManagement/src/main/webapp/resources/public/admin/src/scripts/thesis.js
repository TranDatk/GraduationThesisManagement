...
const loadDataOptions = async (callback) => {
    if (isLoadDataOptions) {
        try {
            const studentsResponse = await fetch("/GraduationThesisManagement/admin/api/student-options", {
                method: "GET",
                headers: {
            ...
            let studentsHtml = "";
            studentsData.forEach((d) => (studentsHtml += `<option value=${d[0]}>${d[1]}</option>`));
            document.getElementById("students").innerHTML = studentsHtml;

            const lecturersResponse = await fetch("/GraduationThesisManagement/admin/api/lecturer-options/?isMinistry=false", {
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
                },
            ...
            let lecturersHtml = "";
            lecturersData.forEach((d) => (lecturersHtml += `<option value=${d[0]}>${d[1]}</option>`));
                ...
                .then((data) => {
                    if (Object.keys(data).length === 0) {
                        ...
                        successfulAlert("Thêm khóa luận thành công", "Ok", () => {
                            // Thực hiện gọi API /send-mail/{lecturerId} sau khi tạo khóa luận thành công
                            fetch(`/GraduationThesisManagement/admin/api/send-mail/${formData.reviewLecturer}`)
                                    .then((response) => {
                                        if (response.ok) {
                                            return response.text(); // Lấy dữ liệu như là một chuỗi văn bản
                                        } else {
                                    ...
                                    .then((data) => {
                                        ...
                                        console.log(data); // Hiển thị thông báo hoặc dữ liệu trả về từ API
                                    })
                                    .catch((error) => {
                                        console.error(error);
                                    });
    ...
    confirmAlert("Bạn có chắc không?", "Bạn sẽ không thể khôi phục điều này!", "Có, xóa nó", "Không, hủy bỏ", () => {
        ...
        fetch(endpoint, {
            method: "DELETE", headers: {
                ...
                .then(function (res) {
                    if (res.status === 204)
                        successfulAlert("Xóa khóa luận thành công", "Ok", () => location.reload());
                })
                .catch((err) => {
